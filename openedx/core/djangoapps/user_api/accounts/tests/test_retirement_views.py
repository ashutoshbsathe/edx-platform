...
class TestPartnerReportingCleanup(ModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('accounts_retirement_partner_cleanup')
        self.maxDiff = None
    ...
    def create_partner_reporting_statuses(self, is_being_processed=True, num=2):
        ...
        for _ in range(num):
            user = UserFactory()
            status = UserRetirementPartnerReportingStatus.objects.create(
                user=user,
                original_username=user.username,
                original_email=user.email,
...
class TestPartnerReportingList(ModuleStoreTestCase):
    ...
    def assert_status_and_user_list(self, users, expected_status=status.HTTP_200_OK):
        """
        ...
        curr_users = response.json()
        print(curr_users)
        print(users)

        self.assertEqual(len(users), len(curr_users))

        # Due to the complexity of the data structure we can't use assertCountEqual, this works around that
        found = False
        for returned_user in users:
            for testing_user in curr_users:
                if returned_user['original_username'] == testing_user['original_username']:
                    ...
                    curr_users.remove(testing_user)
                    break
            self.assertTrue(found)
        self.assertFalse(curr_users)
